...
export default function SidebarWithHeader({ children }) {
    ...
    const fetchProducts = async() => {
        try {
            const res = await axios.get('http://localhost:6099/api/reports/mostsoldproducts', {
                withCredentials: true
            });
            setProducts(res.data.mostSoldProducts);
        } catch(err) {
            console.log(err);
        }
    }
                    ...
                    <Table variant="simple" size="sm" pt={2} >
                        <Thead>
                            <Tr borderBottom="1px" borderColor={'#A1BDD914'}>
                                ...
                                <Th borderBottom='1px' borderRight={'1px'} borderColor={'#A1BDD914'} color='gray.400' textTransform={'none'} py={5}>Numri i shitjeve</Th>
                                <Th borderBottom='1px' borderRight={'1px'} borderColor={'#A1BDD914'} color='gray.400' textTransform={'none'} py={5}>Emri produktit</Th>
                                <Th borderBottom='1px' borderRight={'0px'} borderColor={'#A1BDD914'} color='gray.400' textTransform={'none'} py={5}>Cmimi</Th>
                            </Tr>
                        </Thead>
                        <Tbody >
                            {products.map((product) => (
                                <Tr key={product.id}>
                                    <Td borderRight={'1px'} borderTop='1px' borderBottom={'0px'} borderColor={'#A1BDD914'} color='gray.400' fontWeight={'500'}>{product.productId}</Td>
                                    <Td borderRight={'1px'} borderTop='1px' borderBottom={'0px'} borderColor={'#A1BDD914'} color='gray.400' fontWeight={'500'}>{product.totalSold}</Td>
                                    <Td borderRight={'1px'} borderTop='1px' borderBottom={'0px'} borderColor={'#A1BDD914'} color='gray.400' fontWeight={'500'}>{product.productDetails.name}</Td>
                                    <Td borderRight={'0px'} borderTop='1px' borderBottom={'0px'} borderColor={'#A1BDD914'} color='gray.400' fontWeight={'500'}>{product.productDetails.price}</Td>
                                </Tr>
                            ))}
                        </Tbody>
                    </Table>
